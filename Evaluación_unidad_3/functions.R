...
download_esi_data <- function(url, file_name, directory = "data"){
  ...
  tryCatch({
    download.file(url, dest, mode = "wb", quiet = TRUE, timeout = 300)
  }, error = function(e){
    download.file(url, dest, method = "curl", mode = "wb", quiet = TRUE, timeout = 300)
  })
...
benchmark_strategies <- function(tbl_list){
  ...
  strategy3 <- function(){
    rbindlist(lapply(tbl_list, function(df){
      DT <- as.data.table(df)
      nm <- names(DT)
      col_ing  <- nm[str_detect(nm, "^ing_t|ingreso")][1]
      col_ocup <- nm[str_detect(nm, "ocup")][1]
